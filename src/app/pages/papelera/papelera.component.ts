...
export class PapeleraComponent implements OnInit {
  ...
  load(): void {
    ...
    this.api.getPapelera().subscribe({
      ...
      error: () => { this.alert.set({type:'error',message:'Error cargando papelera'}); this.loading.set(false); }
    });
  }
  ...
  eliminarPermanente(): void {
    const doc = this.confirmDelete(); if (!doc) return;
    this.saving.set(true);
    this.api.eliminarPermanente(doc.id).subscribe({
      next: () => { this.alert.set({type:'success',message:'Documento eliminado permanentemente'}); this.confirmDelete.set(null); this.saving.set(false); this.load(); },
